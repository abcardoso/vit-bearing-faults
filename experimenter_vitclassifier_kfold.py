...
def experimenter_classifier_kfold(
    ...
    if not base_model:
        if pretrain_model:
            ...
            train_and_save(model, pretrain_train_loader, pretrain_eval_loader, num_epochs, lr, saved_model_path)
        else:
            print("Loading pre-trained model.")
            model = load_trained_model(model_class, saved_model_path, num_classes=num_classes).to("cuda")
    else:
